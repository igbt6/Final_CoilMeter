...
void GLCD_InitPorts(void) {
	...
	GPIO ->P[PIN_PORT_STER].MODEH = (GPIO_P_MODEH_MODE8_PUSHPULL
			| GPIO_P_MODEH_MODE9_PUSHPULL | GPIO_P_MODEH_MODE10_PUSHPULL
			| GPIO_P_MODEH_MODE11_PUSHPULL | GPIO_P_MODEH_MODE12_PUSHPULL);
	GPIO ->P[PIN_PORT_DATA].MODEL = (GPIO_P_MODEL_MODE0_PUSHPULL
			| GPIO_P_MODEL_MODE1_PUSHPULL | GPIO_P_MODEL_MODE2_PUSHPULL
			| GPIO_P_MODEL_MODE3_PUSHPULL | GPIO_P_MODEL_MODE4_PUSHPULL
			| GPIO_P_MODEL_MODE5_PUSHPULL | GPIO_P_MODEL_MODE6_PUSHPULL
			| GPIO_P_MODEL_MODE7_PUSHPULL);


/* DEVBOARD
	GPIO->P[PIN_PORT_STER].MODEL = (GPIO_P_MODEL_MODE0_PUSHPULL
			| GPIO_P_MODEL_MODE1_PUSHPULL | GPIO_P_MODEL_MODE2_PUSHPULL
			| GPIO_P_MODEL_MODE3_PUSHPULL | GPIO_P_MODEL_MODE4_PUSHPULL);
	GPIO->P[PIN_PORT_DATA].MODEL = (GPIO_P_MODEL_MODE0_PUSHPULL
			| GPIO_P_MODEL_MODE1_PUSHPULL | GPIO_P_MODEL_MODE2_PUSHPULL
			...
			| GPIO_P_MODEL_MODE7_PUSHPULL);
*/
	GPIO_PortOutSet(PIN_PORT_STER, PIN_ALL_STER_PORT);
	GPIO_PinOutClear(PIN_PORT_STER, PIN_RES);

	for (i = 0; i < 1000; i++)
		// petla opozniajaca+
		;
	GPIO_PinOutSet(PIN_PORT_STER, PIN_RES); // to juz jest w inicjalizacji
...
void GLCD_WriteData(uint8_t dataToWrite) {
	...
	GPIO ->P[PIN_PORT_DATA].MODEL = (GPIO_P_MODEL_MODE0_PUSHPULL
			...
			| GPIO_P_MODEL_MODE3_PUSHPULL | GPIO_P_MODEL_MODE4_PUSHPULL
			| GPIO_P_MODEL_MODE5_PUSHPULL | GPIO_P_MODEL_MODE6_PUSHPULL
			| GPIO_P_MODEL_MODE7_PUSHPULL); // output mode, push pull , max speed 10 MHz

	GPIO_PortOutClear(PIN_PORT_STER, PIN_A0 | PIN_WR | PIN_RD | PIN_CS1);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_A0 | PIN_RD);
	GPIO_PortOutClear(PIN_PORT_DATA, PIN_ALL_DATA_PORT);
	GPIO_PortOutSet(PIN_PORT_DATA, dataToWrite);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_CS1 | PIN_WR); // ustawiam CS1 czyli zakoñczone zapisywanie /czytanie
}
//-------------------------------------------------------------------------------------------------
...
//		; // sprawdzam flage zajetosci
	GPIO_PortOutClear(PIN_PORT_STER, PIN_A0 | PIN_WR | PIN_RD | PIN_CS1);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_RD);
	GPIO_PortOutClear(PIN_PORT_DATA, PIN_ALL_DATA_PORT);
	GPIO_PortOutSet(PIN_PORT_DATA, commandToWrite);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_CS1); // ustawiam CS1 czyli zakoñczone zapisywanie /czytanie
}
//-------------------------------------------------------------------------------------------------
...
uint8_t GLCD_ReadData() {
	...
	GPIO_PortOutClear(PIN_PORT_STER, PIN_RD | PIN_CS1);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_RD | PIN_CS1);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_A0 | PIN_WR); //// tu na tym WR sie zastanowic
	GPIO_PortOutClear(PIN_PORT_STER, PIN_RD | PIN_CS1);
	read_data |= (GPIO ->P[PIN_PORT_DATA].DIN & PIN_ALL_DATA_PORT); // czytam stan wejsæ
...
uint8_t GLCD_ReadStatus(void) {
	/*	GPIO_InitTypeDef GPIO_InitStructure;
	 ...
	 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //wejcie High impedance
	 GPIO_Init(PIN_PORT_DATA, &GPIO_InitStructure);*/
	GPIO ->P[PIN_PORT_DATA].MODEL = (GPIO_P_MODEL_MODE0_INPUT
			| GPIO_P_MODEL_MODE1_INPUT | GPIO_P_MODEL_MODE2_INPUT
			| GPIO_P_MODEL_MODE3_INPUT | GPIO_P_MODEL_MODE4_INPUT
			| GPIO_P_MODEL_MODE5_INPUT | GPIO_P_MODEL_MODE6_INPUT
			| GPIO_P_MODEL_MODE7_INPUT); ///input, --------- w gecko jeden hujasek pull down

	GPIO_PortOutClear(PIN_PORT_STER, PIN_A0 | PIN_WR | PIN_RD | PIN_CS1);
	GPIO_PortOutSet(PIN_PORT_STER, PIN_WR);
	uint8_t read_status = 0;
	read_status = (GPIO ->P[PIN_PORT_DATA].DIN & PIN_ALL_DATA_PORT); // czytam wejscia
	GPIO_PortOutSet(PIN_PORT_STER, PIN_CS1 | PIN_RD | PIN_A0);
